...
\date{\today}
%\begin{abstract}
%Quasi-brittle yielding is a salient feature  of well-annealed glassy   materials. Here we show that  the same behavior is characteristic of  plastic yielding in  model 2D perfect crystals where    massive nucleation of dislocations  can  effectively convert  the atomic configuration from  crystalline  to quasi-amorphous.   To relate  subsequent deformation with  quasi-brittle yielding of amorphous solids  we present a detailed study of  dislocation dynamics  in  a    prototypical square crystal  subjected to quasistatic loading. We show that,  in close parallel to amorphous plasticity,  both pre- and post-yield  avalanches  exhibit power law statistics with  matching exponents  indicative of marginal stability. 
%\end{abstract}
 \maketitle
%\noindent
%
...
%% 
%%\emph{Introduction.} 
%Despite intense studies in recent years,  the phenomenon of  plastic yield  in  crystalline  solids  remains rather mysterious and  
%one of the most challenging  questions  concerns the nature of post-yield plastic flow which often exhibits  intermittent fluctuations and shows  scale free spatial organization ~\cite{Zaiser2006-gk,jian2024prediction,ortiz1999plastic,Papanikolaou2017-ld,kosterlitz2016kosterlitz,khantha1997mechanism,hochrainer2014continuum,langer2019statistical,Derlet2016-mp,ispanovity2010submicron}. These phenomena are also characteristic of amorphous plasticity and  to reinforce the conceptual link,  here we  report a striking similarity between plasticity  of pristine  crystals and quasi-brittle yield in  glassy materials   ~\cite{kamani2024brittle,berthier2024yielding,Shang2020,shang2024yielding,singh2020brittle,richard2021brittle,pollard2022yielding,Lin2014-bx,parisi2017shear,derlet2021micro}.
% 
...
% Specifically, we  show that plastic yielding in stylized 2D  pristine crystals can be represented as a two-stage process.  At the end of the first 'microplasticity'  stage   massive dislocation nucleation results in the formation of what is effectively a glassy dislocational configuration, see also  \cite{Lehtinen2016-qy,Sethna2017,Ovaska2017}.
%The  second stage starts with plastic flow  which can be  viewed as  a superposition of  spatially localized dislocation rearrangements.  This regime   is terminated    abruptly with a    system size event  which 
%  leads to  the   formation of  multi-grain pattern  represented by system-size  shear bands. Similar   quasi-brittle  events are  characteristic    of  plastic yielding in well-annealed glasses  \cite{Nicolas2018-iy,Rodney2011-ld,popovic2018elastoplastic,Tanguy2021-ap,TALAMALI2012275, Parley2024-jb,karmakar2010statistical,ozawa2018random,Divoux2024-ks}. 
%  
%  The claimed similarity  may be surprising because the phenomena of plastic yield in crystals and amorphous solids are  sometimes perceived as fundamentally different because dislocations, the main carriers of plastic deformations  crystals, are absent in amorphous solids. However, the  underlying elementary mechanical events  can be in both cases described  as  triggering of Eshelby-type shear eigenstrains \cite{Picard2004,Budrikis2017-ex,Cai2006-fe} and ultimately  behind both phenomena is an evolving elastic incompatibility   \cite{Moshe2015-rm,Baggioli2022-nx,Bowick2009-bl,Katanaev1992-kl,Sozio2020-tg}.
% 
% To corroborate the analogy  we present here a detailed study of  dislocation dynamics  in  a prototypical  square crystal  subjected to non-generic quasistatic loading by shear strain applied  along one of two the slip directions. Our main tool is a  mesoscopic tensorial model  (MTM) of crystal plasticity representing a conceptual trade-off between continuum  and atomic descriptions~\cite{Salman2011,Salman2012b,Baggio2019,Salman2021,perchikov2024,Baggio2023,Baggio2023-qu,Zhang2020-ax}. Crucially, it   allows one  to  reproduce  intermittent nature of  dislocational avalanches \cite{Weiss2021-tt,Weiss2015-eh}.  The ability of other  existing computational approaches 
...
%to reproduce  the correct statistics of plastic fluctuations, while also accounting for large lattice rotations and adequately representing the crystallographic nature of lattice-invariant shears\cite{Engel1986-tm,Michel2001,schwarzenberger1972classification,Bhattacharya2004,pitteri2002continuum}, remains rather limited. The MTM can be viewed as Landau type theory with infinite number of equivalent energy wells   \cite{Ericksen1970,ericksen1973loading,Ericksen1977,Ericksen1980,ericksen1989weak,parry1976elasticity,Parry1977,Parry1998,Folkins1991,Conti2004-sv,Salman2011,Salman2012b,Baggio2019,Salman2021,perchikov2024,Baggio2023,Baggio2023-qu,Zhang2020-ax}. It can deal adequately with dislocation cores  while operating with engineering concepts of stress and strain and  accounting correctly not only   for long range elastic interactions of dislocations  but also for their  short range interaction  including   topological transitions involved in nucleation and annihilation.  Various related  approaches can be found in \cite{Denoual2010,Gao2019,Gao2020,Gao2020a,Gao2020f,Gao2020c,Biscari2015-ci,Arbib2020,Arbib2023,Arbib2025}.
%
...
%\begin{figure}[h!]
%\centering
%\includegraphics[scale=0.35]{figures_ordering/figure_01.png}
...
%
%%\emph{The model.} 
%In  the MTM   the role of the order parameter  is played by the metric tensor  characterizing local deformation.   Suppose that  
%%\begin{equation}
%$\bf y = \bf y( \bf x)$
%%\end{equation}
%is  the deformation   where   $\bf y$ and $\bf x$  are  positions of material  point  in the current and reference configurations.
%  Due to frame indifference  constraint, the strain energy density of an elastic solid  $\phi$
%  %= \phi(\bf C)$ 
...
%   In the corresponding 2D continuum theory the elastic energy  density  $\phi= \phi(\bf C)$ is assumed to respect the full  symmetry of a Bravais lattices  going   beyond the conventional continuum mechanics point group and requiring   that $ \phi(\bf C) = \phi(\bf m^T \bf C \bf m),$  where $\bf m$ belongs to  $ GL(2,\mathbb{Z}) = \left\{{\bf m},\, m_{IJ}\in{\mathbb Z},\, \det(\bf m)=\pm1 \right\}.$   This implies that such energy density is a multi-well function of tensorial parameter with  the bottoms of the energy wells corresponding to lattice invariant deformations. In view  of the   $ GL(2,\mathbb{Z})$ symmetry, the surface $\det{\bf C}=1$  in the 3D space $(C_{11}, C_{22}, C_{12})$    is tessellated   into an infinite number of equivalent periodicity domains. To better visualize such a tessellation, it is convenient project the infinite surface $\det{\bf C}=1$ stereographically on a (Poincar\'e) disk with unit radius as illustrated in , see Fig.~\ref{fig:atlas0}, where  the equivalent square lattices are indicated by small black squares while red dots denote equivalent triangular (hexagonal) lattices. The structure of the minimal periodicity domain $D$, which is known in the mathematical literature on modular forms as a 'fundamental domain',   is discussed in \cite{SM}.  If the the energy density  is known in one of such   tensorial periodicity domains, it can be extended to the whole configurational space.
%   
%%   As we have already mentioned,   it is sufficient to define the energy density  only in one minimal periodicity  domain,  $D$ which is known in the mathematical literature on modular forms as a 'fundamental domain', see shaded domains in our Fig.~\ref{fig:atlas}(a,b,c). 
%%   
%%   
%
...
%%metric tensor (characterizing local deformation).
%
%
%
%%
%% To express the global lattice induced symmetry of the  function  
%%   %\begin{equation}
%%   $\phi= \phi(\bf C)$
%%   %\end{equation}
%%
%%  In the numerical implementation of the MTM, the space of admissible deformations is reduced to compatible piece-wise-affine mappings. Therefore, the outcome of the   numerical energy
...
%
%%For comparison, we also present in Fig.~\ref{fig:atlas0}c another orthogonal projection of the same surface   on  a  plane.  In both projections the equivalent square lattices are indicated by small black squares while red dots denote equivalent triangular (hexagonal) lattices.
%
%% we   recall that 2D Bravais lattices can be generated by two linearly independent vectors $\lbrace {\bf e}_I\rbrace,\;I=1,2,$ which form a lattice basis.  The two bases ${\bf e}_I$ and ${\bar{\bf e}}_I$ describe the same lattice if 
...
%% %\end{equation}
%%Therefore,  to preserve a Bravais lattice, the strain energy density of a crystal must respect the symmetry 
%% %\begin{equation}
...
%\end{figure} 
%
%Our way of constructing the energy density $\phi(\bf C)$ is discussed in \cite{SM}. In Fig. \ref{fig:atlas1}  we illustrate the resulting energy landscape  in the configurational space of metric tensors $\bf C$ with $\det{\bf C}=1$. It highlights the fact that any such  landscape would have  a network of low energy valleys along the directions representing simple shears parallel to crystallographic slip planes. It also reminds us that   the deepest energy wells  correspond  to the equivalent replicas of the reference square  lattice  furnished  by rank one deformation gradient tensors  representing lattice invariant simple shears. Instead, pure shear deformations  lead to extremely large elastic energy levels (not visible in Fig. \ref{fig:atlas1} because of truncation in the vertical direction).
%
% 
%
...
%
%The effective incorporation of the  quantized lattice invariant shears makes the energy $\phi(\bf C)$ highly  non convex. Therefore,  the corresponding  continuum (scale free) problem is  highly degenerate and needs to be regularized.  To this end the   the   original lattice is effectively coarse-grained with an introduction of a uniform mesoscale grid and the  elastic  response is attributed   to   discrete material elements  whose linear size $h$ is viewed as a  physical parameter of the model.  In this way  we  reduce  the space of admissible deformations to a finite dimensional set of compatible, piece-wise affine mappings, see  \cite{SM}. Driven  quasistatically,   the  ensuing mechanical system   experiences a rich repertoire of   instabilities which are represented by the jumps  representing  dissipative processes.  In this way  elastic  energy minimization    hides   rate independent   plasticity behind the nonlinear elasticity appearance.
% 
%
%% 
...
%
% 
%\emph{Numerical setup. } In our numerical experiments we   started with a pristine (defect free) square crystal sample represented by $N\times N$ finite elements with $N=400$. We then loaded this system quasi-statically along one of the the principal slip direction by  applying an affine displacement field 
%%\begin{equation}
%$ {\bf u}(\alpha,{\bf x})= (\bar{\bf F}(\alpha )-\mathbb{1}){\bf x} $
%% \end{equation}
%which was applied to all  boundary nodes. The   deformation gradient 
...
%  %\end{equation}
%was chosen to represent  simple shear along an  “easy” glide direction with shear amplitude $\alpha$  serving as the loading parameter. 
% By changing this parameter in increments of order $10^{-6}$  we  advanced   the  loading, and after each increment   the  displacement field was updated  by  an appropriate energy minimization algorithm, see \cite{SM} for details.  
% 
...
%In view of the pristine nature of our samples,  the initial phase of the deformation process
...
%%  
%  The aﬃne deformation becomes   unstable at a critical value of the loading parameter $\alpha=\alpha_c\approx0.138$    which can be well approximated  if we assume that the instability is associated with the loss of  positive definiteness of  the acoustic tensor \eqref{Q}, see \cite{Simpson1989,Grabovsky2013,Steigmann2023} and \cite{SM}. 
%%  
...
%   At the critical value of the loading parameter
%   % (point $B$ in Fig. \ref{fig:02}) 
%    all finite elements  become unstable  simultaneously
%%   . This would lead  to a nonphysical highly unstable behavior    and therefore, to observe a generic response we had 
%   and to regularize the problem we   add  at the   onset of instability  a small amount of quenched  disorder which  was removed right after the instability  took place.
%   
%     The breakdown of an   almost  aﬃne elastic state  takes the form of a major  system-size  rearrangement transforming a perfect crystal into a dislocation-rich configuration.
%    %  which we interpret as  'amorphous crystal'. 
%     Specifically,  in the course of this  catastrophic  system-spanning event  the crystal   abruptly loses almost all its  accumulated elastic energy with stress   dropping almost to zero. The  deformation pattern  emerging after the stress drop (point $C$),  reveals   \emph{collective}   nucleation  of a large   number of dislocations $\approx 2000$. The latter were identified  using the classical Delaunay triangulation method, see \cite{SM}. It  allowed us  to detect  the coordination number anomalies and 5/7 coordination defect pairs were interpreted    as dislocations. 
%
%
%  
%%\begin{figure}[ht!]
...
% 
%\begin{figure}[h!]
%\includegraphics[scale=.15]{figures_ordering/figure_101.pdf}
...
% resolved in conventional bulk deformation experiments.  However,  this  small-scale  intermittent  micro-plastic  activity  which usually becomes apparent  only  in samples of  reduced size, is still  characteristic of pre-yield behavior for  both crystal and glassy systems  \cite{Maas2018-qu,Papanikolaou2017-ld,Sparks2018-zp,sparks2019avalanche,Rizzardi2022-iz,Duan2023-ue}. We interpret this  regime as quasi-elastic.
% 
%  The quasi-elastic regime ends with discontinuous transition to   plastic  yielding. 
%%Note  that while the observed catastrophic  mechanical response  is still associated with \emph{plastic} deformation, its  discontinuous nature  is highly reminiscent of brittle fracture. 
%The phenomenon of  plastic brittleness in crystal plasticity is  realistic,  moreover   it has been qualified as  a characteristic feature of sub-micron crystal samples with high initial purity (absence of solutes, precipitates, and dislocations)~\cite{bei2008effects,chrobak2011deconfinement,wang2012pristine,Cui2017-xn,Greer2011-hi,Brenner1956-rx,Brenner1958-dr,Sharma2018-iw,Mordehai2018-qm,Lilleodden2006-jq,Corcoran1997-vt}. Such discontinuous yield  is   is   highly reminiscent of     mechanical  response of well annealed ' brittle' glasses. In particular, we have seen that among other amorphous systems which exhibit similar quasi-brittle behavior    there are  jammed configurations of  granular media which  are only marginally stable. 
%
%
...
%%%\includegraphics[scale=.1]{figures_ordering/figure_12.pdf}
...
%\begin{figure}[h!]
%%\includegraphics[scale=.1]{figures_ordering/figure_11.pdf}
%\includegraphics[scale=.15]{figures_ordering/figure_12.pdf}
%\caption{Second stage yielding represented by the system size restructuring of a dislocation pattern. Major shear band forms between the states $G$ and $H$. Colors indicate the level of energy density. ( \textbf{correct?}) }
%\label{fig:031}
%\end{figure}
%
% 
%To understand such macroscopically quasi-brittle  yield  we now turn to the analysis of the corresponding  spatial dislocational micro-configurations emerging during the associated   restructuring event starting at point $F$.  We   interpret this  transition from quasi-elastic, pre-yield  behavior of an 'amorphopus crystal' to the actual plastic yield.  The whole  event  takes the form  of a two-stage  transition  $F \to G \to H$.The corresponding snapshots of the  spatial dislocational micro-configurations are shown in Fig. \ref{fig:031}.  Here we  already  see the evidence of a collective activity of dislocations which leads to  the formation of  global structures. This trend is mostly pronounced during the abrupt stress drop associated with the transition  $G \to H$ where we observe massive nucleation of dislocations bringing about  a system-spanning shear band   formed  by differently oriented dislocation-free  grains. The  development of such grain texture  marks the initial shift from   'local' to 'global'  dislocational arrangements. During  the  unfolding of the implied system spanning dislocational avalanche the microstructural evolution is dominated by the propagation and growth of the previously created  grain boundaries. One can view  the emerging bands as representing  collections of differently oriented  grains. The corresponding mis-orientations  are controlled by the overall compatibility of the deformation field. In particular, the  ubiquitous presence  in such patterns of  low energy $\Sigma 5$  grain boundaries is  discussed in detail in \cite{Baggio2023-qu}, see also \cite{Sutton1984-us,Priester2012-wx,Randle2024-tf}.
%
%%Finally,  an even more radical rearrangement of the dislocational structure,  bringing about the state $H$,   marks the  coalescence of different grains and points towards  the formation of  at least one extended shear band. 
%
%
% 
%
...
% 
%%According to the collected information,   the yield point (state $F$)  marks  the initial formation of grain boundaries which emerge as a result of  self-organization among  a large number of dislocations. Such self organization becomes possible due to the presence of both, long range elastic interactions and short range interactions induced by dislocation cores. 
%
% \begin{figure}[h!]
%\includegraphics[scale=.18]{figures_ordering/figure_14.pdf}
...
%%Our Fig. \ref{fig:3}, presenting  the snapshots of the strain-energy density field  in the states  $F$ and $G$,  illustrates in more detail the first stage of this critical transition. 
%
%%The emerging low energy grain  boundaries are   dislocation-rich  which  explains the necessity to nucleate a large amount of dislocations during the transition from state $G$ to state $H$. Such dislocational arrangements are   highly optimized  which suggests that at this stage of the deformation history the  self-organization becomes  global.  
%
%We show in  Fig. \ref{grains3} a representative snapshot of the of the strain-energy density field of the crystal in state $H$.   Here again the  regions with higher energy  are  localized around dislocation cores.  The  avalanche  $G \to H$ involves both,  a  growth of an  existing  grain and  its    merger which  another grain.  This  consolidates the structure of the shear band which may be also viewed as an element  of a polycrystaline texture.  The corresponding catastrophic stress drop  may  be explained by the fact that, in view of their highly optimized nature, stresses  required to move grain boundaries are smaller  than those sufficient to move individual dislocations. 
%
...
% 
%\begin{figure}[h!]
%\includegraphics[scale=.16]{figures_ordering/figure_15.pdf}
...
%
%
%%The post-yield fully plastic yielding, represented by the state  $I$,  can be interpreted as quasi-stationary non-equilibrium steady state.  due to relative stabilization of the average stress level. 
%A representative snapshot of the micro-configuration of the crystal in state $I$ is shown in  Fig. \ref{fig:04}.
% The   intermittent avalanches,  characterizing this regime,  represent the subsequent broadening and development of  the polycrystalline grain texture.  While bigger grains  occasionally merge to form larger rotated patches,  small grains continue to nucleate. 
%%which requires an  increase of the number of dislocations. 
%In this way the self-organization of dislocations within the existing shear bands continues while the latter keep  widening, while  the average macroscopic stress remains constant.
...
% Fluctuational activity is known to represent  an important window into  the mechanical  response of crystals undergoing plastic deformation \cite{Papanikolaou2017-ld,Weiss2015-eh,Zaiser2006-gk,Tarp2014-ro,Miguel2006-wd,Alava2014,Sethna2017}. As we have seen,  after our 'preparation' a pristine crystal  is  effectively quenched in  marginally stable configuration which we interpret as a state of  'amorphous crystals'.  The next goal is to study  plastic fluctuations  generated by the resulting  system subjected to quasistatic  driving.  Specifically we  will study   the probability distribution of the energy drops associated with individual  avalanches in  pre-yield regime (exemplified by the state  $E$ ) and post-yield regime  (exemplified by the state   $I$ ). Note that  in amorphous solids the statistics of such energy drops has been found to be represented by a power law with a cut-off that depends on system size \cite{Tyukodi2016-dv,Budrikis2017-ex,Lerner2018-ue,Ozawa2022-xb} of the type
%%\begin{equation}
%$P(x)  \sim  x^{-\epsilon}e^{\lambda x}.$
%%\end{equation}
%In the context of amorphous plasticity such  behavior is usually  described in terms of simple elasto-plastic models 
...
%  \cite{Nicolas2018-iy,PhysRevLett.129.228002,Ferrero2019-rx,Fernandez-Castellanos2021-yn}
%  and while   the MTM  approach to crystal plasticity operates within  very similar finite element settingr, there are also some important differences. Thus, phenomenological  yield thresholds are replaced by elastic instabilities in a given energy landscape and  fixed elastic propagators are replaced by the solution of a nonlinear elasticity problem. It is also important  the MTM approach to deformation is geometrically exact which allows one to  handle adequately finite rotations and elastic compatibility. 
%
%
...
%\begin{figure}[h!]
%\includegraphics[scale=.08]{figures_ordering/figure_17.pdf}
...
%deformation  closely resembles the behavior of similar characteristics in well-annealed glasses  \cite{Papanikolaou2017-ld,Weiss2021-tt,Sethna2001,Ispanovity2014-ra,Alava2014,Lehtinen2016-qy}. Our   results are summarized  in Fig. \ref{fig:energy_drops} where we see that  the statistics of  both pre- and post-(second)yield  avalanches show  the power law range  extending over up to six decades. Remarkably, despite all the  differences in the macroscopic response , both pre-yield and post-yield energy distributions share the same power-law exponent: $\epsilon = 1.0$. This is an indication that  the  scaling behavior  remains basically unchanged across the  yielding transition despite the fact that we observe  hardening in the pre-yield regime
%% where the dislocation number remains basically unchanged 
% and stationary plastic flow in the post-yield regime
...
%  
%  To conclude,   since the developed model deals only with  quasi-statically loaded,  low dimensional,  defect-free model crystals at zero  temperature,  it  effectively underplays  such important physical effects of  realistic crystal plasticity  as dislocation climb, cross slip, dislocation pinning and  forest  hardening,  to mention just a few. Furthermore, only a 3D model resolving the difference between HCP, FCC and BCC symmetry classes will distinguish adequately edge, screw, and mixed dislocations, and  will be suitable to handle   realistic dislocation entanglements.
%  % The 3D model 
...
\paragraph {Fundamental and elastic domains.}  We  refer to the restriction of the energy density to the (minimal periodicity) fundamental domain   as $\phi_D(\tilde{\bf C})$, where $\tilde{\bf C}={\bf m}^T{\bf C} {\bf m}$  is the projection of  a  general metric tensor ${\bf C}$ into this domain and   ${\bf m}$ is the  corresponding  mapping  of the metric tensor ${\bf C}$ into  the fundamental   domain ensuring that  $ \phi({\bf C})= \phi_D(\tilde{\bf C})$.  In our 2D case   the form of the fundamental domain   is known   \cite{Parry1998,Conti2004-sv,Engel2012,pitteri2002continuum}
\begin{equation}
D = \left\{  0<C_{11}\le C_{22},\quad 0\le C_{12}\le \frac{C_{11}}{2}\right\}.
\end{equation}
where $ \det{\bf C}=1$. Given a generic metric $\bf C$, the task of finding a unimodular matrix ${\bf m} $ ensuring that $\tilde{\bf C}\in D$ (and therefore $ \phi({\bf C})= \phi_D(\tilde{\bf C})$) can be formulated in the form of a recursive algorithm which is also well known  (Lagrange reduction) \cite{Conti2004-sv,Engel2012}. Specifically, if we define the matrices   
\begin{equation}
...
%In what follows  we present the results of a direct comparisons between the outcomes of some benchmark numerical experiments conducted in MTM and in MS frameworks.  As a  basis of such comparison we point to the fact that both types of models are based on the same microscopic model  relying on   pair interactions between atoms and  operating with  exactly the same interatomic potential.
 \begin{figure}[h!]
\centering
\includegraphics[scale=0.25]{Figures_ordered/fig_1_SM.pdf}\caption{Strain-energy density landscape in  the the space of shear strains visualized on a  Poincar\'e disk using  stereographic projection. Thin white lines show the original tessellation into equivalent periodicity domains. Colors represents  level of energy. The stability boundaries obtained  from the equation \eqref{detQ}   are shown by a thick black line around the reference energy well. ( \textbf{drop other four boundaries})
%. Five boundaries are shown: the original stability envelope and four copies generated by applying the coordinate transformations $\mathbf{m}^T \cdot \mathbf{C} \cdot \mathbf{m}$ with $\mathbf{m} \in \left\{\begin{bmatrix} 1 & \pm 1 \\ 0 & 1 \end{bmatrix}, \begin{bmatrix} 1 & 0 \\ \pm 1 & 1 \end{bmatrix}\right\}$, representing equivalent energy wells in the metric space.
}
\label{01}
\end{figure}

\paragraph{Elastic energy density.} While the single-period Landau potential $\phi_D(\tilde{\bf C})$ can be   constructed using the classical Cauchy-Born approach, see for instance   \cite{Baggio2023-qu, Baggio2023},  in this paper  we  used for simplicity the  phenomenological expression proposed in \cite{Conti2004-sv}. Specifically, the  elastic energy density is represented as a sum of two terms:
 \begin{equation}
\phi(\tilde{\bf C}) = \phi_0 \left(\frac{\tilde{\bf C}}{(\det {\tilde{\bf C}})^{1/2}} \right) + \phi_v(\det \tilde{\bf C}), 
\end{equation}
where $\phi_0 $ accounts for contributions due to   shear   while $\phi_v$ penalizes    volumetric deformations. The $ GL(2,\mathbb{Z})$ invariant shear  term is chosen in the form  sixth order polynomial  which is the minimal requirement  ensuring   stress continuity over the whole configurational space  \cite{Parry1998,Conti2004-sv}. It depends on a single parameter $\beta$ allowing one to associate   ground state either with square or triangular (hexagonal) lattice 
...
%The total elastic energy, allowing one to deal with both shear  and volumetric deformations, is taken in the form
% \begin{equation}
%\phi(\tilde{\bf C}) = \phi_0 \left(\frac{\tilde{\bf C}}{(\det {\tilde{\bf C}})^{1/2}} \right) + \phi_v(\det \tilde{\bf C}). 
%\end{equation}
...
$ \phi_v(s) = \mu(s - \log(s)). $
% \end{equation}
  To keep in our numerical experiments the strain field close the surface $\det C = 1$, we adopted   a sufficiently high value of the bulk modulus,  $\mu = 5$. 

%. Suppose   that material points in a representative volume $\Omega $ undergo an affine deformation  such that \begin{equation}
...
%One can also achieve a desired level of smoothness phenomenologically,
%
% by representing  the energy density inside the domain  $D$ using  polynomial functions of different degrees. 
...
% Recall first that in molecular dynamics approach (or rather molecular statics  (MS) in our quasi-static loading case)  there is also a characteristic internal scale: 
 
%Introduce an  interatomic distance $a$ and the linear size of the macroscopic domain .  In the mesoscopic approach of the MTM  the   coarse-graining scale  $h=L/N$  defines the  cutoff beyond which the deformation is considered homogeneous (affine), where  $N^2$ is the number  of the nodes in the mesoscopic  grid. 

% In terms of   dimensionless  microscale parameters   $\epsilon=a/L$ and $\delta=h/L$, we  are interested in the limit we are ultimately implying the double limit $\epsilon \to 0$, $\delta \to 0$ while  $\delta\gg\epsilon$. 
% 
 
 

% we effectively "blur"  the core regions  of   dislocations and lower   their number, the remaining  dislocations  


%Then, to obtain a conventional scale free continuum theory we  must perform the second limit $\delta \to 0$. In the MTM approach  we effectively avoid this second limit and maintain a  small but finite value of the parameter $\delta$. In this sense the MTM is essentially a continuum theory with an internal length scale, like, say, phase field model and other similar 'quasi-continuum' approaches. 


%  The assumption  that $\delta$ is finite is equivalent to incorporating  finite size effects.  Thus,  even though by keeping    $\delta $  finite  we effectively "blur"  the core regions  of   dislocations and lower   their number, the remaining  dislocations   preserve the basic nature of  their   interactions  at least far away from the boundaries. In particular, the   'blurred' dislocations   will still   nucleate and self-lock adequately. In our simulations  we adopted  a pragmatic  approach to the choice of the mesoscopic length scale $h$ by requiring  that  the  Cauchy-Born   energy density,  computed for a   finite  lattice fragment with  scale $h$,    can be considered as approximately periodic within  the interesting range of strains. 
 
%   even if  perhaps at different values of controlling parameters than in the corresponding MS simulations. Note also that at least some of the  characteristic   parameters including the core energy, the Peierls stress, the elasticity of a dislocation line, etc.,  may  be  matched by using $\delta$ dependent interatomic potential  which will then affect  the barrier structure in the corresponding  Cauchy-Born energy density.   The  situation here is the same as in the case of phase field description of phase boundaries where the need to reproduce  the realistic  value of the surface energy requires  the microscopic potential  to be appropriately re-scaled. 
%
% 
%
% 
%  
% The use of Cauchy-Born coarse grained  description, will also  lead  to some  loss of   microscopic details in the description of dislocation cores,  however, 

 
 
% 
%  which then necessarily satisfies  the  constraint $\delta\gg\epsilon$. In this situation we have  $a<<h \sim L$ and both $h$ and $L$ survive in the  limit $a/L \to 0$, see  Fig. \ref{01}. 
% 
%%  Essentially we consider  a double   asymptotics 
%%$
%%   \epsilon \to 0,  \delta \to 0 ,
%%$
%%with $\delta\gg\epsilon$ which implies that $a<<h<<L$ and the only remaining length scale is $L$, see Fig. \ref{01}. 
%
%
%Instead, In the MTM approach we effectively consider only a single limit $ \epsilon \to 0$ which  allows us still to use the Cauchy-Born rule. We then
% 
% and  while we are ultimately implying the limit $\epsilon \to 0$, $\delta \to 0$, we must assume that $\delta\gg\epsilon$.  To locate in this perspective  the  classical continuum approach, we recall that  $h$ can be interpreted as an effective  size of a 'continuum particle'.  
% 
%  is necessarily larger than $a$ and smaller than $L$. 
% 
%  since  the goal is to   accelerate the computations. Their relative  magnitudes  should be, of course, discussed in terms of dimensionless quantities.  Thus, if the size of the macroscopic domain is  $L$, its  division  into finite elements introduces a dimensionless internal scale   $ h = L/N $ where we recall that $N^2$ is the number  of the nodes in the mesoscopic finite-element grid.
...
%As on all such theories,  making the dimensional parameter $h$ smaller is equivalent to making another dimensional parameter $L$ larger. In this sense
...
%This means we deal with the all particles in the original central domain
...
(iii)The triangulation naturally incorporates the macroscopic deformation through the deformed positions of the periodic images.
 The selected set of triangles $\mathcal{T}_{\text{unique}}$ forms the computational mesh for the MTM calculationswhile implementing  the periodic boundary conditions and ensuring the consistency with the applied macroscopic deformation gradient $\bar{\bf F}(\alpha )$. 
