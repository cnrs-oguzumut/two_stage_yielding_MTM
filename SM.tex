...
To determine the power-law exponent and the finite-size scaling exponent (often interpreted as the fractal dimension of the avalanches), we employ a robust data-collapse optimization procedure. Near the critical point, the probability distribution $P(s;L)$ of avalanche sizes $s$ (which can represent either the macroscopic stress drop $S$ or the dissipated energy $E$) for a system of linear size $L$ obeys the finite-size scaling (FSS) ansatz:
\begin{equation}
...
\end{equation}
The optimal parameters $(x^*, D_x^*)$ are found by minimizing the residual spread (variance) between the rescaled curves across their common overlapping range. For a discrete set of system sizes $\{L_k\}$, the quality of the collapse at a trial pair $(x,D_x)$ is evaluated systematically by first computing the logarithmic variables $u_k = \log_{10}\tilde{x}_k$ and $v_k = \log_{10}\tilde{y}_k$ for each $L_k$. We then identify the common $\log_{10}\tilde{x}$ interval limits $u_{\min} = \max_k\min(u_k)$ and $u_{\max} = \min_k\max(u_k)$. For our datasets specifically, a robust overlap domain naturally exists near the optimal scaling exponents. Next, we lay down a uniform grid of 80 points $\{c_j\}$ spanning $[u_{\min},u_{\max}]$ and map each curve $v_k$ onto it using linear interpolation. Finally, we define the valid index set $\mathcal{V}$ where all $N_L$ curves provide a finite evaluation, computing the variance of the complete collapse across this set as:
\begin{equation}
  \mathcal{Q}(x,D_x)
  \;=\;
  \frac{1}{|\mathcal{V}|}
  \sum_{j\in\mathcal{V}}
  \mathrm{Var}_k\!\left[v_k(c_j)\right],
  \label{eq:quality}
\end{equation}
where $\mathrm{Var}_k$ evaluates the variance spanning the $N_L$ system sizes at the fixed grid point $c_j$.

Evaluating this collapse variance provides a highly sensitive measure of how well the distributions coincide. A two-stage grid-search (coarse then fine) in the $(x, D_x)$ plane identifies the global minimum of $\mathcal{Q}$, defining the optimal $(x^*, D_x^*)$. Uncertainties in the exponents are estimated directly from the curvature of the $\mathcal{Q}$ landscape near its minimum. By fitting a parabola to the projected 1-D profiles of $\mathcal{Q}$, the 1-$\sigma$ uncertainty is defined as the half-width at which the variance metric doubles from its minimum value, logically analogous to a maximum-likelihood $\Delta \chi^2$ confidence interval. A sharp, deep minimum in the variance spread thus demonstrates that the parameters are tightly constrained by the data overlap.
